...
:Description: A preliminary design and analysis tool for single-seat
              ultralight aircraft under FAA FAR Part 103. This application
              provides real-time performance estimates, weight & balance
              calculations, and stability feedback for various types of
...
class AlulaApp(tk.Tk):
    def __init__(self):
        ...
        self.FAR_103_MAX_FUEL_LBS = self.FAR_103_MAX_FUEL_GAL * 6
        self.FAR_103_MAX_SPEED_KNOTS = 55
        self.FAR_103_STALL_SPEED_KNOTS = 24
        self.RHO_SEA_LEVEL_SLUG = 0.002377
        self.HELIUM_DENSITY_SLUG = 0.000332
        self.KNOTS_TO_FPS = 1.68781
    ...
    def create_widgets(self):
        ...
        self.sizing_tab_widgets, self.aero_tab_widgets = {}, {}
        tab_funcs = {"Configuration": self.create_config_tab, "Sizing": self.create_sizing_tab, "Weights": self.create_weights_tab, "Aerodynamics": self.create_aero_tab, "Issues & Feedback": self.create_feedback_tab}
    ...
    def create_right_panel(self, parent):
        ...
        self.results_desc_labels = {}
        self.results_value_labels = {}
        sections = {"Weights (Estimated)": ["Est. Empty Weight:", "Max Gross Weight:", "Max Fuel Weight:"], "Loadings": ["Wing Loading:", "Power Loading:", "Span Loading:"], "Performance (Estimated)": ["Stall Speed Clean:", "Stall Speed Flaps:", "Max Level Speed (VH):", "Rate of Climb (ROC):"], "Center of Gravity (CG)": ["Longitudinal CG:", "Est. Static Margin:", "Calculated CG Location:"]}
    ...
    def create_sizing_tab(self, parent):
        inputs = {'wing_area': "Wing Area (sq ft):", 'wing_span': "Wing Span (ft):", 'aspect_ratio': "Aspect Ratio (Flat):", 'fuselage_length': "Fuselage Length (ft):", 'lemac_ft': "MAC Leading Edge (ft):", 'rotor_diameter': "Rotor Diameter (ft):", 'rotor_blade_chord': "Rotor Blade Chord (ft):", 'num_blades': "Number of Blades:", 'envelope_volume': "Envelope Volume (cu ft):"}
        self.create_dynamic_input_tab(parent, inputs, self.sizing_tab_widgets)

    def create_aero_tab(self, parent):
        inputs = {'cl_max': "Max Lift Coeff (Cl_max):", 'cl_max_flaps': "Max Lift Coeff (Flaps):", 'cd0': "Base Zero-Lift Drag (Cd0):", 'neutral_point_ft': "Wing Neutral Point (ft):", 'engine_hp': "Engine Power (HP):", 'prop_efficiency': "Propeller Efficiency (0-1):", 'oswald_efficiency': "Oswald Efficiency (e):", 'rotor_rpm': "Rotor RPM:", 'rotor_blade_cd': "Rotor Blade Cd (profile):"}
    ...
    def create_weights_tab(self, parent):
        ...
        components = [("Wing", "60", "4.5"), ("Fuselage", "50", "8.5"), ("Empennage", "15", "16"), ("Engine & Mount", "45", "1.0"), ("Landing Gear", "25", "4.0"), ("Fuel System", "5", "1.5"), ("Misc Systems", "15", "6.0")]
        self.component_entries = []
        for _, (name, weight, arm) in enumerate(components):
            name_var, weight_var, arm_var = tk.StringVar(value=name), tk.StringVar(value=weight), tk.StringVar(value=arm)
            ttk.Entry(parent, textvariable=name_var).grid(row=len(self.component_entries)+1, column=0, padx=5, pady=2, sticky='ew')
            ttk.Entry(parent, textvariable=weight_var).grid(row=len(self.component_entries)+1, column=1, padx=5, pady=2)
            ttk.Entry(parent, textvariable=arm_var).grid(row=len(self.component_entries)+1, column=2, padx=5, pady=2)
    ...
    def update_ui_for_vehicle_type(self):
        ...
        sizing_visibility = {'Fixed Wing': ['wing_area', 'wing_span', 'fuselage_length', 'lemac_ft'], 'Glider': ['wing_area', 'wing_span', 'fuselage_length', 'lemac_ft'], 'Gyrocopter': ['rotor_diameter', 'rotor_blade_chord', 'num_blades', 'fuselage_length'], 'Helicopter': ['rotor_diameter', 'rotor_blade_chord', 'num_blades', 'fuselage_length'], 'Lighter Than Air': ['envelope_volume', 'fuselage_length'], 'Paraglider': ['wing_area', 'aspect_ratio']}
        aero_visibility = {'Fixed Wing': ['cl_max', 'cl_max_flaps', 'cd0', 'neutral_point_ft', 'engine_hp', 'prop_efficiency', 'oswald_efficiency'], 'Glider': ['cl_max', 'cl_max_flaps', 'cd0', 'oswald_efficiency'], 'Gyrocopter': ['rotor_blade_cd', 'cd0', 'engine_hp', 'prop_efficiency', 'rotor_rpm'], 'Helicopter': ['rotor_blade_cd', 'cd0', 'engine_hp', 'rotor_rpm'], 'Lighter Than Air': ['cd0', 'engine_hp', 'prop_efficiency'], 'Paraglider': []}
        ...
        for i, key in enumerate(sizing_visibility.get(v_type, [])):
        ...
        for i, key in enumerate(aero_visibility.get(v_type, [])):
    ...
    def update_all_calculations(self):
        v_type = self.data['inputs']['vehicle_type'].get()
        total_weight, total_moment, pwr_sys_w = 0, 0, 0
        for entry in self.component_entries:
            try:
                w, a = float(entry['weight'].get()), float(entry['arm'].get())
                total_weight += w; total_moment += w * a
                if 'engine' in entry['name'].get().lower(): pwr_sys_w += w
        ...
        self.data['calculations'] = {"Empty Weight": empty_weight, "Gross Weight": gross_weight, "Fuel Weight": fuel_weight, "Power System Weight": pwr_sys_w, "CG Location": cg_location, "Pilot Weight": pilot_weight}
        calc_map = {'Fixed Wing': self.calculate_fixed_wing, 'Glider': self.calculate_glider, 'Gyrocopter': self.calculate_gyrocopter, 'Helicopter': self.calculate_helicopter, 'Lighter Than Air': self.calculate_lta, 'Paraglider': self.calculate_paraglider}
        calc_function = calc_map.get(v_type)
        if callable(calc_function):
            calc_function()
    ...
    def calculate_fixed_wing(self, is_glider=False):
        ...
        if not is_glider:
            ...
            if vs_fps > 0:
                for v_fps in range(int(vs_fps), 300):
                    Cl = (2 * gross_weight) / (self.RHO_SEA_LEVEL_SLUG * wing_area * (v_fps**2))
                    Cd = total_cd0 + k * (Cl**2)
                    power_req = 0.5 * self.RHO_SEA_LEVEL_SLUG * (v_fps**3) * wing_area * Cd
                    if power_req > power_avail: vh_fps = float(v_fps - 1); break
            max_excess_power = -float('inf')
            climb_v_end = int(vh_fps * 1.05) if vh_fps > vs_fps else int(vs_fps * 1.5)
            for v_fps in range(int(vs_fps * 1.05), climb_v_end):
                Cl = (2 * gross_weight) / (self.RHO_SEA_LEVEL_SLUG * wing_area * v_fps**2)
                Cd = total_cd0 + k * (Cl**2)
                power_req = 0.5 * self.RHO_SEA_LEVEL_SLUG * (v_fps**3) * wing_area * Cd
                excess_power = power_avail - power_req
                if excess_power > max_excess_power: max_excess_power = excess_power
            roc_fpm = (max_excess_power / gross_weight) * 60 if gross_weight > 0 and max_excess_power > -float('inf') else 0.0
            calc.update({"Wing Loading": gross_weight / wing_area if wing_area > 0 else 0, "Power Loading": gross_weight / engine_hp if engine_hp > 0 else 0, "Span Loading": gross_weight / wing_span if wing_span > 0 else 0, "Stall Speed": vs_fps / self.KNOTS_TO_FPS, "Stall Speed Flaps": vs_flaps_fps / self.KNOTS_TO_FPS if self.data['inputs']['flaps'].get() else vs_fps / self.KNOTS_TO_FPS, "VH": vh_fps / self.KNOTS_TO_FPS, "ROC": roc_fpm if roc_fpm > 0 else 0})
        else:
            calc.update({"L/D Max": ld_max, "Min Sink Rate": min_sink_fps * 60, "Speed @ Min Sink": v_ld_max_fps / self.KNOTS_TO_FPS, "Stall Speed": vs_fps / self.KNOTS_TO_FPS, "Stall Speed Flaps": vs_flaps_fps / self.KNOTS_TO_FPS if self.data['inputs']['flaps'].get() else vs_fps / self.KNOTS_TO_FPS, "Wing Loading": gross_weight / wing_area if wing_area > 0 else 0, "VH": v_ld_max_fps / self.KNOTS_TO_FPS * 1.5})
        mean_chord = wing_area / wing_span if wing_span > 0 else 1
        calc.update({"Static Margin": ((np_ft - calc["CG Location"]) / mean_chord) * 100 if mean_chord > 0 else 0, "CG MAC Percent": ((calc["CG Location"] - lemac_ft) / mean_chord) * 100 if mean_chord > 0 else 0, "Total Cd0": total_cd0, "Base Cd0": base_cd0, "Cockpit Drag": cockpit_drag, "Tail Drag": tail_drag})

    def calculate_glider(self): self.calculate_fixed_wing(is_glider=True)
    ...
    def calculate_paraglider(self):
        ...
        aero_props = self.paraglider_class_map.get(glider_class)
        if not aero_props: return
        ...
        min_sink_fps = v_ld_max_fps / ld_max if ld_max > 0 else 0
        calc.update({"L/D Max": ld_max, "Min Sink Rate": min_sink_fps * 60, "Trim Speed": trim_speed_fps / self.KNOTS_TO_FPS, "Stall Speed": vs_fps / self.KNOTS_TO_FPS, "Wing Loading": gross_weight / wing_area if wing_area > 0 else 0, "VH": trim_speed_fps * 1.4 / self.KNOTS_TO_FPS, "Static Margin": "N/A", "CG MAC Percent": "N/A"})
    ...
    def calculate_rotorcraft(self, is_helicopter):
        ...
        cockpit_drag = self.cockpit_drag_map.get(self.data['inputs']['cockpit_style'].get(), 0)
        fuselage_drag_area = (base_cd0 + cockpit_drag) * 15
        ...
        min_speed_fps = 15 * self.KNOTS_TO_FPS
        vh_fps = 0.0
        if is_helicopter:
            ...
            for v_fps_int in range(1, 250):
                ...
                if power_req_fwd > power_avail: vh_fps = v_fps - 1; break
        else: # Gyrocopter
            roc_fpm = 0.0
            rotor_drag_area = rotor_area * 0.05
            ...
            for v_fps_int in range(1, 250):
                ...
                if thrust_req > thrust_avail: vh_fps = v_fps - 1; break
        calc.update({"Disc Loading": gross_weight / rotor_area if rotor_area > 0 else 0, "Power Loading": gross_weight / engine_hp if engine_hp > 0 else 0, "Tip Speed": tip_speed, "Min. Fwd Speed": min_speed_fps / self.KNOTS_TO_FPS, "VH": vh_fps / self.KNOTS_TO_FPS, "ROC": roc_fpm if roc_fpm > 0 else 0, "Static Margin": "N/A", "CG MAC Percent": "N/A"})

    def calculate_helicopter(self): self.calculate_rotorcraft(is_helicopter=True)
    def calculate_gyrocopter(self): self.calculate_rotorcraft(is_helicopter=False)
    ...
    def calculate_lta(self):
        ...
        calc.update({"Buoyant Lift": buoyant_lift, "Net Lift": net_lift, "Static Heaviness": "Heavy" if net_lift < 0 else "Light", "VH": vh_fps / self.KNOTS_TO_FPS, "ROC": "N/A", "Stall Speed": "N/A", "Static Margin": "N/A", "CG MAC Percent": "N/A"})

    def _set_result_value(self, original_text, new_text, calc_key, unit, compliance_val=None):
        """Helper to set a single line in the results panel."""
    ...
    def update_cg_canvas(self):
        ...
        margin = 30
        marker_color = 'white'
        if isinstance(sm, (int, float)) and not (5.0 <= sm <= 15.0): marker_color = self.style.lookup('Red.TLabel', 'foreground')
    ...
    def update_pie_chart(self):
        ...
        struct_w = empty_weight - pwr_sys_w
        fractions = {"E": struct_w / gross, "V": pwr_sys_w / gross, "F": calc.get("Fuel Weight", 0) / gross, "P": calc.get("Pilot Weight", 0) / gross}
        ...
        for name, frac in sorted_fractions:
            extent = frac * 360
            if extent > 0.5: self.pie_canvas.create_arc(cx - radius, cy - radius, cx + radius, cy + radius, start=start_angle, extent=extent, fill=colors[name], outline='white')
    ...
    def update_flight_envelope(self):
        ...
        vne = max(vh * 1.1, vh + 10)
        margin_l, margin_r, margin_t, margin_b = 60, 50, 20, 50
        max_g, min_g, max_v = 4.5, -2.5, max(100, vne * 1.1)
        def to_canvas(v, g):
            x = margin_l + (v / max_v) * (w - margin_l - margin_r)
            y = (h - margin_b) - ((g - min_g) / (max_g - min_g)) * (h - margin_t - margin_b)
            return x, y
        _, origin_y = to_canvas(0, 0)
        canvas.create_line(margin_l, margin_t, margin_l, h - margin_b, fill='grey')
        canvas.create_line(margin_l, origin_y, w - margin_r, origin_y, fill='grey')
        ...
        canvas.create_text(margin_l - 30, h / 2, text="Load Factor (g)", fill="white", angle=90)
        ...
        canvas.create_line(pos_stall_pts, fill='#E87B33', width=2)
        canvas.create_line(p_va_pos, p_vne_pos, fill='#E87B33', width=2)
        canvas.create_line(neg_stall_pts, fill='#4A90E2', width=2)
        canvas.create_line(p_vne_pos, p_vne_neg, fill='#FF00FF', width=2)
        ...
        legend_items = [("Clean Stall / Limit", "#E87B33", "solid"), ("Flaps Stall", "#87CEEB", "dashed"), ("Negative Stall / Limit", "#4A90E2", "solid"), ("Vne Limit", "#FF00FF", "solid"), ("Operating Envelope", "#4A4A4A", "fill")]
    ...
    def update_feedback_tab(self):
        ...
        if isinstance(sm, (int, float)):
            if 5 <= sm <= 15: feedback.append("✔️ Pitch Stability: Good. Static margin is in the ideal 5-15% range.")
            else: feedback.append("❌ Pitch Stability: Poor. Static margin is outside the ideal range. Check CG and Neutral Point.")
        weight_limit = self.FAR_103_GLIDER_EMPTY_WEIGHT_LBS if v_type in ['Glider', 'Paraglider'] else self.FAR_103_EMPTY_WEIGHT_LBS
        if calc.get("Empty Weight", 0) > weight_limit: feedback.append(f"❌ Compliance: Empty weight exceeds Part 103 limit for this aircraft type ({weight_limit} lbs).")
        else: feedback.append(f"✔️ Compliance: Empty weight is within Part 103 limit for this aircraft type ({weight_limit} lbs).")
        ...
        if isinstance(wl, (int, float)):
            if wl < 2.5: feedback.append("ℹ️ Handling: Very low loading suggests high sensitivity to turbulence.")
            elif wl > 5.0: feedback.append("ℹ️ Handling: High loading suggests higher landing speeds.")
    ...
    def save_design(self):
        ...
        data_to_save = {'main_inputs': {key: var.get() for key, var in self.data['inputs'].items()}, 'component_weights': [{'name': e['name'].get(), 'weight': e['weight'].get(), 'arm': e['arm'].get()} for e in self.component_entries]}
        try:
            with open(filepath, 'w', encoding="utf-8") as f: json.dump(data_to_save, f, indent=4)
        except (IOError, TypeError) as e: messagebox.showerror("Save Error", f"Failed to save file:\n{e}")
        ...
        try:
            ...
            if 'main_inputs' in loaded_data:
                for key, value in loaded_data['main_inputs'].items():
                    if key in self.data['inputs']: self.data['inputs'][key].set(value)
            if 'component_weights' in loaded_data:
                for loaded_comp, entry_comp in zip(loaded_data['component_weights'], self.component_entries):
                    entry_comp['name'].set(loaded_comp.get('name', '')); entry_comp['weight'].set(loaded_comp.get('weight', '0')); entry_comp['arm'].set(loaded_comp.get('arm', '0'))
            self.update_ui_for_vehicle_type()
        except (IOError, json.JSONDecodeError, KeyError) as e: messagebox.showerror("Load Error", f"Failed to load or parse file:\n{e}")

    def show_about_dialog(self):
        about_win = tk.Toplevel(self)
        about_win.title("About ALULA"); about_win.geometry("350x200"); about_win.resizable(False, False)
        about_win.configure(bg=self.style.lookup('TFrame', 'background')); about_win.transient(self); about_win.grab_set()
    ...
    def show_rules_dialog(self):
        ...
        rules_win.title("FAA FAR Part 103 Summary"); rules_win.geometry("450x250"); rules_win.resizable(False, False)
        rules_win.configure(bg=self.style.lookup('TFrame', 'background')); rules_win.transient(self); rules_win.grab_set()
        ...
        rules = {"Max Empty Weight (Powered):": f"{self.FAR_103_EMPTY_WEIGHT_LBS} lbs", "Max Empty Weight (Unpowered):": f"{self.FAR_103_GLIDER_EMPTY_WEIGHT_LBS} lbs", "Max Fuel Capacity:": f"{self.FAR_103_MAX_FUEL_GAL} U.S. Gallons", "Max Speed (level flight):": f"{self.FAR_103_MAX_SPEED_KNOTS} knots (63 mph)", "Max Stall Speed (power-off):": f"{self.FAR_103_STALL_SPEED_KNOTS} knots (28 mph)", "Occupants:": "Single (1)"}
